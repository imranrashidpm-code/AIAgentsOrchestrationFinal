"""CLI for integration & BFF agent (`int_bff_patterns`)."""
...
from integration_bff_agent import AGENT_OUTPUT_SUBDIR, DISPLAY_NAME, INTEGRATION_BFF_AGENT_IDS, run_integration_bff_agent
...
def main() -> int:
    ...
    p.add_argument("agent_id", nargs="?", default="int_bff_patterns")
    p.add_argument("--list-agents", action="store_true")
    p.add_argument("--prompt", default=os.environ.get("INT_BFF_USER_PROMPT", ""))
    p.add_argument("--constraints", default=os.environ.get("INT_BFF_CONSTRAINTS", "None specified."))
    p.add_argument(
        "--context", dest="business_context", default=os.environ.get("INT_BFF_BUSINESS_CONTEXT", "None specified.")
    )
    p.add_argument("--out-dir", metavar="DIR")
    p.add_argument("--out", metavar="FILE")
    args = p.parse_args()
    if args.list_agents:
        for aid in sorted(INTEGRATION_BFF_AGENT_IDS):
            print(aid)
        return 0
    if not (args.prompt or "").strip():
        print("Provide --prompt.", file=sys.stderr)
        return 2
    if args.agent_id not in INTEGRATION_BFF_AGENT_IDS:
        return 2
    if not os.environ.get("OPENAI_API_KEY"):
        print("Warning: OPENAI_API_KEY is not set.", file=sys.stderr)
    r = run_integration_bff_agent(
        args.agent_id,
    ...
    if args.out_dir:
        sub = AGENT_OUTPUT_SUBDIR.get(args.agent_id, "platform/integration")
