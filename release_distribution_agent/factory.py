...
from .config_loader import load_rel_agents, load_rel_tasks
from .registry import RELEASE_DISTRIBUTION_AGENT_IDS

_PACK = "release_distribution_agent"
...
def run_release_distribution_agent(agent_id: str, inputs: dict[str, str]) -> Any:
    if agent_id not in RELEASE_DISTRIBUTION_AGENT_IDS:
        raise ValueError(
            f"Unknown release/distribution agent_id '{agent_id}'. Valid: {sorted(RELEASE_DISTRIBUTION_AGENT_IDS)}",
        )
    ...
    up = (inputs.get("user_prompt") or "").strip() or (
        "Plan weekly releases for a React PWA and native iOS/Android with staged rollout and feature flags."
    )
