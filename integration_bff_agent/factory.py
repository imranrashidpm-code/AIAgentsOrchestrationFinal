...
from .config_loader import load_int_agents, load_int_tasks
from .registry import INTEGRATION_BFF_AGENT_IDS

_PACK = "integration_bff_agent"
...
def run_integration_bff_agent(agent_id: str, inputs: dict[str, str]) -> Any:
    if agent_id not in INTEGRATION_BFF_AGENT_IDS:
        raise ValueError(
            f"Unknown integration/BFF agent_id '{agent_id}'. Valid: {sorted(INTEGRATION_BFF_AGENT_IDS)}",
        )
    ...
    up = (inputs.get("user_prompt") or "").strip() or (
        "Design a BFF for a React web app and mobile apps on top of a modular monolith and Stripe webhooks."
    )
