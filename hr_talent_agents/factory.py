...
from .config_loader import load_hrt_agents, load_hrt_tasks
from .registry import HR_TALENT_AGENT_IDS
...
def run_hr_talent_agent(agent_id: str, inputs: dict[str, str]) -> Any:
    if agent_id not in HR_TALENT_AGENT_IDS:
        raise ValueError(
            f"Unknown hrt agent_id '{agent_id}'. Valid: {sorted(HR_TALENT_AGENT_IDS)}",
        )
    ...
    up = (inputs.get("user_prompt") or "").strip() or (
        "No user prompt. Produce a draft + checklist the HR/people team can adapt with legal/ER review."
    )
