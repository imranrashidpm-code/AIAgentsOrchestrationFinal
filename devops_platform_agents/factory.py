...
from .config_loader import load_dvp_agents, load_dvp_tasks
from .registry import DEVOPS_PLATFORM_AGENT_IDS

_PACK = "devops_platform"
...
def run_devops_platform_agent(agent_id: str, inputs: dict[str, str]) -> Any:
    if agent_id not in DEVOPS_PLATFORM_AGENT_IDS:
        raise ValueError(
            f"Unknown dvp agent_id '{agent_id}'. Valid: {sorted(DEVOPS_PLATFORM_AGENT_IDS)}",
        )
    ...
    up = (inputs.get("user_prompt") or "").strip() or (
        "No user prompt. Produce a practical template the team can adapt for this DevOps & platform workstream."
    )
