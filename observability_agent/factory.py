...
from .config_loader import load_obs_agents, load_obs_tasks
from .registry import OBSERVABILITY_AGENT_IDS

_PACK = "observability_agent"
...
def run_observability_agent(agent_id: str, inputs: dict[str, str]) -> Any:
    if agent_id not in OBSERVABILITY_AGENT_IDS:
        raise ValueError(
            f"Unknown observability agent_id '{agent_id}'. Valid: {sorted(OBSERVABILITY_AGENT_IDS)}",
        )
    ...
    up = (inputs.get("user_prompt") or "").strip() or (
        "Propose OTel-based metrics, logs, and traces for a microservices + React + iOS/Android stack with checkout SLOs."
    )
