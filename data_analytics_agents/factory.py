...
from .config_loader import load_dta_agents, load_dta_tasks
from .registry import DATA_ANALYTICS_AGENT_IDS

_PACK = "data_analytics"
...
def run_data_analytics_agent(agent_id: str, inputs: dict[str, str]) -> Any:
    if agent_id not in DATA_ANALYTICS_AGENT_IDS:
        raise ValueError(
            f"Unknown dta agent_id '{agent_id}'. Valid: {sorted(DATA_ANALYTICS_AGENT_IDS)}",
        )
    ...
    up = (inputs.get("user_prompt") or "").strip() or (
        "No user prompt. Produce a data & analytics template the team can adapt for definitions and design."
    )
