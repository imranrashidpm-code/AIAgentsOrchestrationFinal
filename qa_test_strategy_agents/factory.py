...
from .config_loader import load_qts_agents, load_qts_tasks
from .registry import QA_TEST_STRATEGY_AGENT_IDS

_PACK = "qa_test_strategy"
...
def run_qa_test_strategy_agent(agent_id: str, inputs: dict[str, str]) -> Any:
    if agent_id not in QA_TEST_STRATEGY_AGENT_IDS:
        raise ValueError(
            f"Unknown qts agent_id '{agent_id}'. Valid: {sorted(QA_TEST_STRATEGY_AGENT_IDS)}",
        )
    ...
    up = (inputs.get("user_prompt") or "").strip() or (
        "No user prompt. Produce a reusable QA & test strategy template the team can tailor."
    )
